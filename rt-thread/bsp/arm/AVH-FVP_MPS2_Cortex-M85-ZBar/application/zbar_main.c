...
int get_qrcode_text(void *raw, int width, int height, char *qrcode)
{
    uint16_t i,j;
    ...
    if(zbar_test((void* )pic_data, qr_img_width, qr_img_height, qrcode) < 0) {
        printf("zbar failed \n");
    } else {
        printf("zbar ok \n");
    }
...
/*  
    ...
    printf("qrcode: %s\n", qrcode);*/
    memset(qrcode, 0, sizeof(qrcode));
    get_qrcode_text((void *)gImage_test8, 128, 128, qrcode);
    printf("qrcode: %s\n", qrcode);
    memset(qrcode, 0, sizeof(qrcode));
    get_qrcode_text((void *)gImage_test9, 128, 128, qrcode);
    printf("qrcode: %s\n", qrcode);
