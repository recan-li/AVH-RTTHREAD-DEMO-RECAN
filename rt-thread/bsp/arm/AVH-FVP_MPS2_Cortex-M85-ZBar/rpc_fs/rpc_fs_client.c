...
cJSON *rpc_fs_cJSON_exhcange(cJSON *in)
{
...
RPC_FILE * rpc_fs_fopen(const char *pathname, const char *mode)
{
    ...
    cJSON *rsp_root = rpc_fs_cJSON_exhcange(root);
    if (rsp_root) {
        file = (RPC_FILE *)&g_fd;
        cJSON_Delete(rsp_root);
    }    
...
int rpc_fs_file_size(const char *file_name)
{
...
{    
    ...
    int size = rpc_fs_file_size("123.txt");
    printf("size: %d\n", size);
    size = rpc_fs_file_size("456.txt");
    printf("size: %d\n", size);
