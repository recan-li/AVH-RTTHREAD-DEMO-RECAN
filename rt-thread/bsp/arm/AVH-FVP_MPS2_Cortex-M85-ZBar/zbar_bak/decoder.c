...
#include "zbar.h"
#include "config.h"
#include <stdlib.h>     /* rt_malloc, rt_calloc, rt_free */
...
{
    zbar_decoder_t *dcode = rt_calloc(1, sizeof(zbar_decoder_t));
    dcode->buf_alloc = BUFFER_MIN;
    dcode->buf = rt_malloc(dcode->buf_alloc);
                                    ...
                                    unsigned int buflen)
{
    ...
    if(!decoder_dump || dumplen > decoder_dumplen) {
        if(decoder_dump)
            rt_free(decoder_dump);
        decoder_dump = rt_malloc(dumplen);
