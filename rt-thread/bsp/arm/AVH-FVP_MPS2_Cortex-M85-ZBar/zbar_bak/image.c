...
{
    zbar_image_t *img = rt_calloc(1, sizeof(zbar_image_t));
    _zbar_refcnt_init();
...
void _zbar_image_rt_free (zbar_image_t *img)
{
    if(img->syms) {
...
zbar_image_t *zbar_image_copy (const zbar_image_t *src)
{
    ...
    dst->data = rt_malloc(src->datalen);
    assert(dst->data);
                          ...
                          const char *filename)
{
    ...
    rows = rt_malloc(img->height * sizeof(*rows));
    if(!rows)
        goto done;
