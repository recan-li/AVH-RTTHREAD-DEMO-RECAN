...
{
    zbar_video_t *vdo = rt_calloc(1, sizeof(zbar_video_t));
    if(!vdo)
        return(NULL);
    ...
    vdo->images = rt_calloc(ZBAR_VIDEO_IMAGES_MAX, sizeof(zbar_image_t*));
    if(!vdo->images) {
    ...
    if(img) {
        ...
        if(vdo->num_images < 2) {
            ...
            if(!img) {
                ...
                img->height = vdo->height;
                img->datalen = vdo->datalen;
                img->data = rt_malloc(vdo->datalen);
            }
