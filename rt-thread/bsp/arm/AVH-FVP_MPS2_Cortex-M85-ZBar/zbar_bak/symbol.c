...
void _zbar_symbol_rt_free (zbar_symbol_t *sym)
{
    if(sym->syms) {
...
{
    zbar_symbol_set_t *syms = rt_calloc(1, sizeof(*syms));
    _zbar_refcnt(&syms->refcnt, 1);
    return(syms);
}

inline void _zbar_symbol_set_rt_free (zbar_symbol_set_t *syms)
{
                          ...
                          int delta)
{
    ...
    if(!_zbar_refcnt(&ncsyms->refcnt, delta) && delta <= 0)
        _zbar_symbol_set_rt_free(ncsyms);
}
