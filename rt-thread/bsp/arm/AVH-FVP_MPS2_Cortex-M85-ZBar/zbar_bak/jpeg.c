...
{
    j_decompress_ptr cinfo = rt_calloc(1, sizeof(struct jpeg_decompress_struct));
    if(!cinfo)
        return(NULL);

    errenv_t *jerr = rt_calloc(1, sizeof(errenv_t));
    if(!jerr) {
        rt_free(cinfo);
        return(NULL);
    }
                              ...
                              const zbar_format_def_t *srcfmt)
{
    ...
    /* setup input image */
    if(!cinfo->src) {
        cinfo->src = rt_calloc(1, sizeof(zbar_src_mgr_t));
    ...
    if(!dst->data) {
        dst->datalen = datalen;
        dst->data = rt_malloc(dst->datalen);
        dst->cleanup = zbar_image_rt_free_data;
    }
    else
