...
{
    proc_waiter_t *waiter = proc->rt_free_waiter;
    if(waiter) {
        proc->rt_free_waiter = waiter->next;
        waiter->events = 0;
    }
    else {
        waiter = rt_calloc(1, sizeof(proc_waiter_t));
...
{
    if(waiter) {
        waiter->next = proc->rt_free_waiter;
        proc->rt_free_waiter = waiter;
    }
}
