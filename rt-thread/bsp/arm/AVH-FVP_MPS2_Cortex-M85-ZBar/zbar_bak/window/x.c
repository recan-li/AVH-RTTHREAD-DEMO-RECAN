...
static inline int window_hide_cursor (zbar_window_t *w)
{
    ...
    Xrt_freeCursor(w->display, cursor);
    Xrt_freeGC(w->display, gc);
    Xrt_freePixmap(w->display, empty);
    return(0);
}
                         ...
                         unsigned long win)
{
    ...
    if(w->display) {
        ...
        if(x->gc)
            Xrt_freeGC(w->display, x->gc);
        assert(!x->exposed);
        if(x->font) {
            Xrt_freeFont(w->display, x->font);
            x->font = NULL;
        }
        if(x->logo_zbars) {
            XDestroyRegion(x->logo_zbars);
    ...
    if(!x)
        x = w->state = rt_calloc(1, sizeof(window_state_t));
