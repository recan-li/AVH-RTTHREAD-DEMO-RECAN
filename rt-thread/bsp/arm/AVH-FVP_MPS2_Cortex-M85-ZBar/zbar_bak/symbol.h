...
extern void _zbar_symbol_rt_free(zbar_symbol_t*);

extern zbar_symbol_set_t *_zbar_symbol_set_create(void);
extern void _zbar_symbol_set_rt_free(zbar_symbol_set_t*);
                                  ...
                                  int y)
{
    ...
    if(++sym->npts >= sym->pts_alloc)
        sym->pts = rt_realloc(sym->pts, ++sym->pts_alloc * sizeof(point_t));
    sym->pts[i].x = x;
    sym->pts[i].y = y;
}
